...
Public Module JwcPageAnalizer
    ...
    Friend Async Function GetGenericPageItems(GetFirst As Boolean, URL As String) As Task(Of IEnumerable(Of TextLinkPair))
        Dim xm = Await PageAnalizer.OpenXmlReaderAsync(URL)
    ...
    Public Async Function GetDownloadLink(PageHRef As String) As Task(Of String)
        Dim rd = Await PageAnalizer.OpenXmlReaderAsync(PageHRef)
        With rd
            .Read()
            .ReadToFollowing("a")
            .ReadToFollowing("a")
...
或者下载链接有问题.")
    ...
    Public Function GetAnnouncementsSrcCode(ResponseStream As Stream) As String
        ...
        For i = 1 To 512
            writer.Write("garb")
        Next
        Dim rd = PageAnalizer.OpenXmlReader(ms)
        With rd
            ms.Position = 0
            .Read()
            For i = 1 To 6
